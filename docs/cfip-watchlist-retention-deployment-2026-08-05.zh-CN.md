...
## 2026-08-28 全链路复核、部署漂移修复与修复后重测

### 专家审核结论

- IT 工程专家组按 SRE、安全、工程可验证性和最小变更原则审核通过：只处理 VM36 的同步脚本部署漂移，不改 DNS、门槛、PassWall、cron、Sidecar 或容器。
- Action Gate 判定为低风险；要求先备份、锁门控、哈希校验和本机计划测试，失败自动恢复，不允许绕过真实 PassWall 和单记录写入限制。

### 根因

- VM36 复核时运行的 `sidecar-auto-sync.sh` 哈希为 `4b2f66...`，对应仓库较旧的 `17c5abe`；当前仓库已是 `0763f8d`，脚本哈希为 `ab50e4...`。
- 旧版本缺少完整的主槽重复去重规划：主槽存在重复时，不能把满足真实门控的安全竞争候选纳入替换池，因此 `auto`/`auto1` 可能继续重复。
- 这不是 DNS 污染。Cloudflare 只读内容与局域网解析一致，公共 DNS 的一次短暂不一致随后恢复。

### 工具层修复

- 回归包装器在从 Git tar 包解压后显式恢复 `.sh` 执行权限，修复 `router-candidate-gate.sh: Permission denied` 假失败。
- PowerShell 远端脚本传输统一使用无 BOM、LF 和 Base64；嵌套 SSH 无 stdin 时增加 `-n`，避免吞掉后续部署步骤；需要 heredoc 的 SSH 保留 stdin。
- 一次临时探针曾误用未送入授权 stdin 的 `sudo`，产生 3 次失败认证；未认证成功、未执行 root 命令、未改动生产状态，随后已停止并废弃该路径。

### VM36 最小部署

- 部署前确认 VM36 锁空闲、PassWall 四监听和 HTTP 健康。
- 先在 root-only 备份目录保存旧脚本、配置、cron 和状态文件，再原子替换同步脚本。
- 新脚本哈希：`ab50e4...`；回滚目录：`/root/openwrt-backup/cfip-primary-dedupe-priority-fix-20260828-230724`。
- PassWall 重启 0 次，cron 未变化，Cloudflare/DNS 写入 0 次；主槽晋升开关保持原值。

### 修复后完整测试

- Linux 全量回归再次通过：项目回归、锁协调、候选门控、竞争槽计划、主槽去重计划和 Sidecar 全套测试均通过。
- VM36 四个生产脚本哈希、四个监听、Xray、锁、cron、Cloudflare 只读 GET 和三路 DNS 后检通过。
- `.110` 容器均 running/healthy，Ollama 当前无驻留模型，无 Xray 临时配置和瞬时 CFIP 容器；8 月 27 日的失败仍是历史出口漂移，当前出口矩阵已恢复为预期的同出口关系。

### 尚待自然周期确认

- 当前 `auto` 与 `auto1` 仍保持原有重复值，因为本次部署禁止人工改写 Cloudflare；必须等待下一次既定 04:35 同步，由新脚本按门控最多更新一条记录。
- 当前 `.110` 最新成功报告仍为 `2026-08-26 20:10 UTC`，下一次自然 Sidecar 周期完成后，才可确认出口恢复和主槽去重实际生效。
- 在该自然周期完成前，结论为“代码和部署修复完成，生产效果等待自然验证”，不提前宣称项目完全收口。
