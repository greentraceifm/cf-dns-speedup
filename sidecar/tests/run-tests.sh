...
bash -n "$ROOT/sidecar/integration/sub2api-auto-cleanup.sh"
...
bash "$ROOT/sidecar/tests/test_xray_residue_guard.sh"
bash "$ROOT/sidecar/tests/test_host_cleanup_policy.sh"
bash "$ROOT/sidecar/tests/test_resource_contracts.sh"
grep -q 'SIDECAR_REQUIRE_DIFFERENT_PUBLIC_IP=1' "$ROOT/sidecar/cfip-sidecar.env.example"
grep -q 'SIDECAR_HOST_EXIT_RELATION=different' "$ROOT/sidecar/cfip-sidecar.env.example"
...
grep -q 'chmod 0600 "$CONFIG_DIR/sidecar.env"' "$ROOT/sidecar/install-sidecar.sh"
grep -q '^SIDECAR_HOST_MAINTENANCE_LOCKS=' "$ROOT/sidecar/cfip-sidecar.env.example"
grep -q 'missing_or_mismatched' "$ROOT/sidecar/cfip-sidecar.sh"
